...
const Bio = () => {
  ...
  return (
    <BioContainer>
      <h1>{header}</h1>
      <Description>
        Queremos animarte a crear tu producto y vamos a ser tus mentores en este
        camino. Ya sea montar una startup, o abrir un restaurante, te ayudamos
        a&nbsp;lanzarte.
      </Description>
      <InLineLinksContainer>
        <Link to={`/${randomEspisode}`}>
          <PillLinkComposer text="Escúchanos" />
        </Link>
        <FeaturedLinkComposer color="black" href={twitter} text="Síguenos" />
      </InLineLinksContainer>
    </BioContainer>
  );
};
